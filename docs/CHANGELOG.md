...
*   **Storage Location Removal & Instant Purge System:** Added a high-speed database purge pipeline (`POST /system/purge-backup-location`) that directly deletes indexed files, FTS triggers, processed OCR text, virtual folder links, and AI records matching a target storage path in $<1$ second. Replaced browser native `window.confirm` dialogs with a custom styled confirmation modal offering clear **"Yes (Purge Database)"** and **"No (Keep Database Entries)"** options. Includes strict safety guards preventing root/blank directory purges, and clear UI notices ensuring users know physical files on disk/NAS are never modified or deleted.
*   **Directory Path Resolution Bug Fix:** Updated `resolve_db_path()` in `database.py` to automatically detect when a folder or directory path (e.g. `X:\WABS\database`) is passed rather than a `.db` file, automatically appending `/archive.db`. This eliminates SQLite `(sqlite3.OperationalError) unable to open database file` errors when selecting database folders in Settings and prevents unnecessary fallbacks to offline standby mode.
*   **Database Cursor & Streaming Error Resolution on Aborted Requests:** Resolved `sqlite3.ProgrammingError: Cannot operate on a closed database` and `GeneratorExit` errors in `search.py`, `files.py`, `virtual_folders.py`, and `people.py` caused by streaming generators when clients disconnect or navigate away mid-query. Queries are executed cleanly within database session scopes and returned via standard JSON responses with cache-control headers.
*   **Search Tree View Breadcrumbs & Folder Selection Synchronization:** Updated `FolderTree.jsx` and `Explorer.jsx` so that when searching in tree view, the folder containing the first matching search result is automatically highlighted and selected in the tree, ensuring that breadcrumbs, the folder tree, and file cards remain strictly in sync.
*   **External, Network & Encrypted Storage Resilience (Offline Standby Mode):** Resolved startup failure (`FileNotFoundError: [WinError 3] The system cannot find the path specified: 'X:\\'`) when the database is stored on a network-mapped drive, NAS, or encrypted container volume (e.g. VeraCrypt) that is unmounted at application launch. Instead of crashing or freezing, WABS boots instantly (<0.5s) in configuration mode using a persistent local standby schema (`database/standby.db`), serving valid `200 OK` responses for all startup endpoints (`/directories`, `/stats`, `/tags`, `/virtual-folders`) without throwing `no such table: files` errors.
*   **Dynamic Database Switching & Runtime Reconnection:** Implemented dynamic database engine rebinding via `reconnect_database()` in `database.py`. Users can update or reconnect to their database and thumbnail cache paths directly from Settings without restarting WABS. When pointing to an existing database, WABS loads all archive records and sidecars immediately; when pointing to a new path, it initializes the database schema automatically.
*   **Offline Drive Detection Banner & 1-Click Reconnect:** Added an offline warning banner to the Settings screen displaying the inaccessible path, paired with a direct **"Reconnect / Reload Database Now"** action button. Upon reconnecting over local or high-latency network/SMB storage, WABS displays a live progress toast and logs all reconnect and data rehydration milestones (`[Database] [RECONNECT]`), automatically rehydrating files, directories, virtual folders, people, tags, and dashboard in sequence without requiring manual browser refreshes.
*   **Crash-Proof Search & FTS5 Resilient Fallback:** Fixed ASGI application crashes during search queries on network/encrypted storage caused by SQLite FTS5 shared-memory contention (`sqlite3.OperationalError: disk I/O error` or `vtable constructor failed: files_fts`). Wrapped search query execution in `try ... except` with an automatic fallback to standard SQL `LIKE` searching across filenames, tags, and paths, guaranteeing search streams never drop or terminate the server. Wrapped `search_suggestions` (`files_fts_vocab`) in a protective boundary to eliminate 500 errors during search input.
*   **SQLite Busy Timeout for Remote & Network Volumes:** Added `PRAGMA busy_timeout = 30000` to both initial startup and reconnected database engines, allowing SQLite up to 30 seconds to acquire file locks on busy, remote, or network-mapped volumes before reporting errors.
*   **Windows Detached GUI Logging Stream Protection:** Initialized `sys.stdout` and `sys.stderr` safety fallbacks prior to module imports in `run.py` and `database.py`, completely eliminating `AttributeError: 'NoneType' object has no attribute 'write'` errors when running in detached mode or under `pythonw`.
...
*   **SQLite Connection Exhaustion & QueuePool Timeout Fix (`NullPool`):** Resolved `sqlalchemy.exc.TimeoutError: QueuePool limit of size 5 overflow 10 reached, connection timed out, timeout 30.00` on large databases (230k+ files) and remote/network volumes. Replaced static QueuePool with `NullPool` on database engines (`create_engine(..., poolclass=NullPool)`), eliminating connection leaks and starvation across concurrent scanner, indexer, search, and UI endpoints.
*   **Instant Object Tags Loading via FTS5 Vocabulary Table (`files_fts_vocab`):** Overhauled `get_object_tags()` in `tags.py` to query the SQLite FTS5 vocabulary table (`SELECT term FROM files_fts_vocab WHERE term LIKE 'object:%'`) instead of performing heavy 230k-row table scans across `files`. This cut tag query response times from over 45 seconds down to under 50 milliseconds, eliminating UI loading freezes for Tags and Explorer.
*   **Search Query Unterminated Quote & Special Character Sanitization:** Fixed backend crashes and frontend blank screens (`sqlite3.OperationalError: unterminated string`) when searching for terms with unbalanced quotes or folder names (e.g. `Nithin Priya We"`). Token processing now trims stray outer quotes and properly escapes internal quotes (`clean_word.replace('"', '""')`) for SQLite FTS5 match parameters while preserving accurate substring matching on folder paths and filenames.
*   **Automatic View Cache Invalidation on Reconnect and Unlock:** Updated `App.jsx` and `useExplorer.jsx` so that reconnecting a database or unlocking with Master PIN automatically invokes `explorer.invalidateViewCache()`, ensuring the Dashboard, Explorer, and Search immediately show fresh data without requiring page refreshes.
*   **Settings Banner Clearing & Streamlined Status:** Fixed persistent database offline banner in Settings so that successful reconnection immediately clears `database_offline` state, refreshes settings, and dismisses the alert without requiring manual browser reloads.
*   **Application Logging Diagnostics for Detached Mode:** Enhanced root logger configuration in `main.py` to route all application events to `PrintLogger` when file logging is enabled, ensuring user-visible diagnostics are retained in log files when reporting issues.
