...
*   **Face Scanner $O(N)$ Matrix Optimization:** Optimized the scanning loop to maintain a running 2D NumPy array (`new_embs_matrix`) for in-memory matching instead of calling `np.vstack` on every face check. This avoids quadratic data-copying overhead, keeping the scanning loop constant-time and fast throughout the scan.
