...
*   **Offline Drive Detection Banner & 1-Click Reconnect:** Added an offline warning banner to the Settings screen displaying the inaccessible path, paired with a direct **"Reconnect / Reload Database Now"** action button. Upon reconnecting, WABS automatically rehydrates the entire application state (`loadFiles()`, `loadDirectories()`, `loadVirtualFolders()`, `loadPeople()`, `loadTags()`, `loadDashboard()`) and redirects to the Dashboard, ensuring all records and media appear immediately without requiring an application lock/unlock cycle.
