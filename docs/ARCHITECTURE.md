...
* **In-Memory Optimization:** Pre-fetches existing file paths into an O(1) lookup set during startup, drastically speeding up update scans by avoiding sequential database queries.
...
### 8. Document Text Extraction & Search
To make the contents of PDFs, documents, and code files searchable without bloating the database, WABS utilizes a highly optimized text extraction background worker.
* **Heavy Filtering & Keyword Optimization:** Instead of dumping entire multi-page documents into the index, the text extractor strips noise, removes common stop words, and isolates the top most frequent meaningful words alongside important alphanumeric codes (like invoice numbers, dates, or serials).
* **Instant Search Integration:** These distilled keywords are injected directly into a dedicated SQLite FTS5 index, ensuring that massive documents are completely searchable instantly while keeping the overall database memory footprint tiny and performant.
* **Unified Scanning Depth:** A user-configurable scanning depth setting (`low`, `medium`, and `high`) dynamically scales extraction limits across all supported document types (e.g., PDF pages, Word paragraphs, PPT slides, Excel rows, and Plain Text character counts) to control indexing depth.
