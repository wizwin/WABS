...
def _sanitize_ai_search_query(reply: str) -> str:
    ...
    # 1. Handle JSON response if tiny LLM returned JSON object
    ...
    # 4. Handle multi-line output if tiny model added conversational filler
    lines = [line.strip() for line in clean.splitlines() if line.strip()]
    candidate_line = clean
    if len(lines) > 1:
        for line in lines:
            lower_l = line.lower()
            if lower_l.startswith(("here is", "this query", "note:", "explanation:", "you can use", "i have translated", "search result:")):
                continue
            ...
            for line in lines:
                lower_l = line.lower()
                if not lower_l.startswith(("here is", "this query", "note:", "explanation:", "you can use", "i have")):
                    candidate_line = line
                    break
            else:
    ...
    # 5. Remove common leading prefixes generated by LLMs
    prefix_patterns = [
        r'^(?:query|search\s*query|wabs\s*query|result|output|search|translated\s*query|command)\s*[:=-]\s*',
    ...
    # 6. Strip enclosing quotes ("..." or '...')
    if (clean.startswith('"') and clean.endswith('"')) or (clean.startswith("'") and clean.endswith("'")):
        clean = clean[1:-1].strip()
        
    # 7. Strip trailing explanations if any remain
    if "\n" in clean:
        clean = clean.split("\n")[0].strip()
...
def _is_tiny_model(model: str) -> bool:
    ...
    m_lower = model.lower()
    tiny_patterns = [
        "1b", "1.5b", "2b", "3b", "0.5b", "mini", "tiny", "small", "phi-", "phi3", "phi4-mini",
...
def _get_system_prompt(is_tiny: bool = False) -> str:
    if is_tiny:
        return """Translate search request into WABS query syntax. Output ONLY the query on 1 line.
Prefixes: person:<name> (person:"John Doe"), rel:<relation> (rel:spouse), object:<food/object> (object:pizza, object:car), type:<category/ext> (type:photo, type:video, type:document, type:code, type:audio, type:pdf), date:<YYYY/YYYY-MM>, size:<op><val> (size:>5mb), length:<op><val> (length:>3m), aspect:<landscape/portrait>, resolution:<4k/1080p>, camera:<camera>, genre:<genre>, artist:<artist>, tag:<tag>. Use '+' require, '-' exclude, quotes for phrases ("New York").
Examples:
User: photos of trees on canon in 2024 -> object:tree camera:canon date:2024
User: Amazon invoices scanned in 2023 -> type:document tag:ocr Amazon invoice date:2023
User: Alice and Bob eating sushi in Tokyo -> person:Alice person:Bob object:sushi Tokyo type:photo
User: 4K drone video from 2023 -> type:video resolution:4k drone date:2023
User: Python FastAPI code -> type:code *.py FastAPI
Output ONLY raw query."""
...
def generate_search(payload: dict = Body(...)):
    ...
    system_prompt = _get_system_prompt(is_tiny=is_tiny)
    max_tokens = 100 if is_tiny else 200

    def do_request(sys_p: str, max_t: int):
        req_data = json.dumps({
            "model": model,
            "messages": [
    ...
    try:
        cleaned_query = do_request(system_prompt, max_tokens)
        return {"success": True, "query": cleaned_query}
    except urllib.error.HTTPError as e:
        err_msg = e.read().decode("utf-8", errors="ignore")
        # If failure was context length or model size on extended prompt, fallback to compact prompt automatically
        if not is_tiny and e.code in (400, 413, 422) and any(kw in err_msg.lower() for kw in ("context", "token", "length", "too large", "maximum")):
            try:
                compact_prompt = _get_system_prompt(is_tiny=True)
                cleaned_query = do_request(compact_prompt, 100)
                return {"success": True, "query": cleaned_query}
            except Exception:
                pass
        raise HTTPException(status_code=e.code, detail=f"HTTP {e.code}: {err_msg}")
